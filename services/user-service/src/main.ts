...
import { EventBus, EventTypes, userRegisteredSchema } from '@ecom/events';
...
async function main(): Promise<void> {
  ...
  const { app, profileService } = buildApp({ prisma, bus, logger });

  // Provision a profile whenever the auth service registers a user.
  await bus.subscribe({
    queue: 'user-service.events',
    bindings: [EventTypes.UserRegistered],
    handler: async (envelope) => {
      const payload = userRegisteredSchema.parse(envelope.payload);
      await profileService.createFromRegistration(payload);
    },
  });
